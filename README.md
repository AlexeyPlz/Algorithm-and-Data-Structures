...
## Глава 1. База
### Тема 1.1 Трансляция
Перед исполнением любая программа, написанная на языке программирования, должна быть преобразована в программу на низкоуровневом «машинном» языке — в последовательность инструкций, понятных вычислительному устройству.
- Транслятор - это программа или техническое средство, преобразующее программу с одного языка на другой.
- Трансляция - это преобразование программы с одного языка на другой.

В результате трансляции может быть получен код исполняемой машинной программы или может быть получен интерпретируемый код.
Разделяют два типа трансляции и трансляторов:
- Интерпретатор — программа, выполняющая покомандный анализ, обработку и выполнение исходного кода программы.
- Интерпретация — процесс чтения и выполнения программы.
- Компилятор — программа, переводящая исходный код в программный модуль на низкоуровневом языке.
- Компиляция - анализ и трансл ирование кода программы в набор машинных команд, либо в байт-код. 

Обычно выделяют два вида интерпретаторов:
- Простой интерпретатор - читает код и сразу его исполняет.
- Интерпретатор компилирующего типа - читает код и создаёт в памяти его промежуточное представление — транслирует исходную программу, после этого исполняет получившийся низкоуровневый код.
### Тема 1.2 Парадигмы программирования
...
### Тема 1.3 SOLID
Для объектно-ориентированного программирования сформулированы пять базовых принципов, каждый из которых описывает целый класс шаблонов и подходов.  
Названия этих принципов объединены в аббревиатуру - SOLID.
...
Как и в случае с принципом единственной ответственности, цель принципа разделения интерфейса заключается в минимизации побочных эффектов (side effects) и повторов за счёт разделения ПО на независимые части.
...
На практике этот принцип означает, что лучше наследоваться от высокоуровневых классов, чем от их потомков — тогда изменение в одном из классов не повлечёт лавину последствий в других классах;  
высокоуровневые модули (подпрограммы) не должны импортировать сущности из низкоуровневых.
Этот принцип используется для уменьшения связанности в программах: одни модули можно легко заменять другими, всего лишь меняя модуль зависимости, и никакие перемены в низкоуровневом модуле не повлияют на высокоуровневый.
